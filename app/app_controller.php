...
class AppController extends Controller {
        ...
        var $uses     = array('User','Admin','ProductType','Project','CompanyType','CompanyToCategory','NonProfitType','Coinset','Access','Iteam','Link','LinkAddress');
...
/*  
   ...
   *  Description : This function is used for users permission

   * Created On   : 16-02-11 (00:15am)

   */   

function check_user_permissions($user_type_id = 0,$is_top)
  {	
       ...
       $permissions = $this->Access->find('all',array('conditions' => array('Access.user_type' => $user_type_id)));
       if(!empty($permissions))
       {
          foreach ($permissions as $row) {
              $iteams = $this->Iteam->find('all',array('conditions' => array('Iteam.id' => $row['Access']['iteam_id'],'Iteam.is_top'=>$is_top)));
			  foreach($iteams as $iteam)
			  {			  
              	$permissionsArray[][$iteam['Iteam']['name']] = array('c_name'=>$iteam['Iteam']['c_name'],'f_name'=>$iteam['Iteam']['f_name']);
			  }
          }
       }
	...
	function replace_srt_code($strContent)
  {
     $matches = array();
     $start = "{@_@}=";
     $end = "={@_@}";
     $regex = "/$start([a-zA-Z0-9_\s\-]*)$end/";
    if(preg_match_all($regex, $strContent, $matches))
    {
      if(count($matches))
      {  
        $links = $this->Link->find('first',array('conditions' => array('Link.created_link' => $matches[0][0])));
        ...
        { 
          ...
          $newLink = "<a href='".$redirect."' target='_blank' onclick='saveClickInfo(\"".$links['Link']['visual_text']."\")'>".$links['Link']['visual_text']."</a>"; 
          
          $strContent = str_replace($matches[0][0],$newLink,$strContent);   
        } 
      }
    }  
    return $strContent;
...
function contacttypedropdown($projectid=null) {
       ...
       $typeCondition = $projectid ? " ContactType.project_id='".$projectid."'" : " ContactType.project_id='0'";  

   	    $contacttypedata  =  $this->ContactType->find("all", array('conditions' => array('ContactType.active_status' => '1','ContactType.delete_status' => '0',$typeCondition),'order'=>'ContactType.created ASC','fields'=>array('ContactType.id','ContactType.project_id','ContactType.contact_type_name'))); 
...
/*	
	 ...
	 * 	function    : correspondentcompanytypedropdown()
	...
	function correspondentcompanytypedropdown($projectid='')

	{
       ...
       Configure::write('debug',0);
    	...
    	$this->CompanyType->bindModel(array('hasOne' => array('CompanyTypeCategory' => array('foreignKey' => false,'conditions' => array('CompanyTypeCategory.id = CompanyType.company_type_category_id' )))));
     	...
     	$conditions = "CompanyType.active_status='1' AND CompanyType.is_3rdparty ='1'  AND CompanyType.delete_status='0' ".$typecond;
...
/*	
	...
	function getprojectcontactbycompany($companyid ='')

	{
       ...
       $this->Contact  =    &new Contact();
	    ...
	    $conditions = " Contact.delete_status = '0' AND Contact.active_status = '1' AND Contact.company_id=".$companyid;	       
	...
	function getprojectnotebycompany($companyid ='')

	{
       ...
       $this->Note  =    &new Note();
	    ...
	    $conditions = "Note.company_id=".$companyid;	       

   	   $notedata  =  $this->Note->find("all", array('conditions' => $conditions)); 
