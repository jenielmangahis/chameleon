<?php
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		...
		$match = "";
		  $filesize = $_FILES['data']['size']['donors']['adimage'];
		if($filesize > 0){	
			$filename = strtolower($_FILES['data']['name']['donors']['adimage']);
			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					...
					if($ext==end($file_ext)){
						...
						if(!file_exists($save)){
							list($width_orig, $height_orig) = getimagesize($_FILES['data']['tmp_name']['donors']['adimage']);
							if($maxH == null){
								if($width_orig < $maxW){
									$fwidth = $width_orig;
								}else{
							...
							switch($filetype){
								case "gif":
									$image = @imagecreatefromgif($_FILES['data']['tmp_name']['donors']['adimage']);
								break;
								case "jpg":
									$image = @imagecreatefromjpeg($_FILES['data']['tmp_name']['donors']['adimage']);
								break;
								case "jpeg":
									$image = @imagecreatefromjpeg($_FILES['data']['tmp_name']['donors']['adimage']);
								break;
								case "png":
									$image = @imagecreatefrompng($_FILES['data']['tmp_name']['donors']['adimage']);
								break;
							}
							...
							$errorList[]= "IMAGE ALREADY EXISTS";
						}	
					}
				}		
			}
	...
	$filesize_image = $_FILES['data']['size']['donors']['adimage'];
	if($filesize_image > 0){
