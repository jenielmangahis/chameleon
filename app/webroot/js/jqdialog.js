...
(function($) {
	...
	$.jqDialog = {
		...
		confirm: function(message, callback_yes, callback_no, flag) {
			t.create(message);
			...
			if(flag==1){
				t.parts.bt_yes.show();
				t.parts.bt_no.show();
				t.parts.bt_save.hide();
				t.parts.bt_return_to_page.hide();
			}else{
				t.parts.bt_save.show();
				t.parts.bt_return_to_page.show();
				t.parts.bt_yes.hide();
				t.parts.bt_no.hide();
			}
			...
			t.parts.bt_save.unbind().click( function() {
				t.cleanKeypressCallbacks();
				t.close();
				if(callback_yes) callback_yes();
			});
			...
			t.parts.bt_return_to_page.unbind().click( function() {
				t.cleanKeypressCallbacks();
				t.close();
				if(callback_no) callback_no();
			});
		...
		init: function() {
			...
			t.parts.bt_save = $("<button>").attr({ id: t.ids.bt_save }).append( t.labels.save );
			t.parts.bt_return_to_page = $("<button>").attr({ id: t.ids.bt_return_to_page }).append( t.labels.return_to_page );
			...
			t.parts.div_box.append( t.parts.div_content )
					.append(
						t.parts.div_options.append(t.parts.bt_yes)
										   ...
										   .append(t.parts.bt_save)
										   .append(t.parts.bt_return_to_page)
