		...
		if($_SERVER['HTTP_HOST']=="192.168.1.225:8219"){
			...
			{	unset($_SESSION['fbCounter']);
				...
				{ 
					...
					$me = $facebook->api('/me');
					$me1 = $facebook->api('/me/friends');
					$me3=$facebook->api("/me", array(
 							'fields' => 'picture',
 							'type' => 'large'
 						));	
					$likes = $facebook->api('/me/likes'); 
					$feeds = $facebook->api('/me/feeds'); 
				      print_r($me); print_r($me1);   print_r($me3); print_r($likes); die;
			
print_r($likes);	//$this->Session->write('FacebookUser', serialize($me));
...
<p class="coinBoxBot"><?php echo $html->image('/img/'.$project_name.'/coinBox_RhtBot.gif', array('class'=>'right'));?></p>
			...
			<?php }?>
				...
				<div style="margin:0 auto;width:400px;display:<?php if($status=='Verified') echo "none"; else echo "block";?>" >
						...
						<?php echo $form->end();?>
				</div><br/><br/><b style="margin-left: 240px;">Any item with a</b><span class="red">*</span><b>requires an entry.</b>
			<?php if($session->check('Message.flash')){ $session->flash(); } ?> </div>					
