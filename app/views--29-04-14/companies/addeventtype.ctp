...
<div class="titlCont1"><div style="width:960px;margin:0 auto">
        <?php echo $form->create("Companies", array("action" => "addeventtype",'name' => 'addeventtype', 'id' => "addeventtype",'onsubmit' => 'return validateeventtype();'))?>
        <?php  echo $form->hidden("EventType.id", array('id' => 'id','value'=>isset($event_type_id)?$event_type_id:'')); ?>
        <div align="center" id="toppanel" >
            <?php  echo $this->renderElement('new_slider');  ?>
        </div>
        <?php if(isset($event_type_id)!="") $head="Edit"; else $head="Edit"; ?>
        <span class="titlTxt"><?php echo $head;?> Event Types </span>
        <div class="topTabs">
            <ul>
                <li><button type="submit" value="Submit" class="button" name="data[Action][redirectpage]"><span>Save</span></button></li>
                <li><button type="submit" value="Submit" class="button" name="data[Action][noredirection]"><span>Apply</span></button></li>
                <li><button type="button" id="saveForm" class="button"  ONCLICK="javascript:(window.location=baseUrl+'companies/event_types')"><span> Cancel</span></button></li>
            </ul>
            </ul>
        </div>
    </div></div>
