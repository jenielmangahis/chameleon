...
<?php echo $form->select("$modelname.state",$statedropdown,$selectedstate,array('id' => 'combobox','style'=>'width:230px;'),"---Select---"); ?>
...
// 	<?php  //$nost = count($statedropdown);?>
// 	  source:[<?php //foreach($statedropdown as $dtt=>$key){?>"<?php  //echo $statedropdown[$dtt];?>",<?php //$i++;?><?php //}?>]
// });
  });
  </script>
	<style>
	...
	(function( $ ) {
		$.widget( "ui.combobox", {
			_create: function() {
				...
				var input = this.input = $( "<input id='newst'>" )
					.insertAfter( select )
					.val( value )
					.autocomplete({
						delay: 0,
