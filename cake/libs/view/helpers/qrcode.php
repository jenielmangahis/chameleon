...
class QrcodeHelper extends AppHelper {     
	 ...
	 */
	function text($text = '', $options = array(), $img_dest='') {
        $imgpath=$this->base_url . urlencode($text) . $this->_optionsString($options);
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	}
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function url($url = '', $options = array(), $img_dest='') { //DebugBreak();
        
		$url = Router::url($url, true);
        $imgpath=$this->base_url . urlencode($url) . $this->_optionsString($options); 
        if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function email($mail = '', $options = array(), $img_dest='') {
        $imgpath=$this->base_url . urlencode('mailto:' . $mail) . $this->_optionsString($options);
        if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function telephone($phone = '', $options = array(), $img_dest='') {
        $imgpath=$this->base_url . urlencode('tel:' . $phone) . $this->_optionsString($options); 
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 */
	function contact($contact = array(), $options = array(), $img_dest='') {
		$ret = 'MECARD:';
		...
		$imgpath = $this->base_url . urlencode($ret) . $this->_optionsString($options);
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param array $options options array, see helper vars for description of parameters
	 */
	function sms($number = '', $options = array(), $img_dest='') {
        $imgpath = $this->base_url . urlencode('sms:' . $number) . $this->_optionsString($options);  
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        } 
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param array $options options array, see helper vars for description of parameters
	 */
	function mms($number = '', $options = array(), $img_dest='') {
         $imgpath = $this->base_url . urlencode('mms:' . $number) . $this->_optionsString($options);   
          if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
		return $this->Html->image($imgpath, array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function geo($geo = array(), $options = array(), $img_dest='') {
		if (!isset($geo['lat'])) {
			$geo['lat'] = '';
		}
		if (!isset($geo['lon'])) {
        ...
        $imgpath = $this->base_url . urlencode('geo:' . $geo['lat'] . ',' . $geo['lon'] . ',' . $geo['height']) . $this->_optionsString($options); 
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }  
		return $this->Html->image($imgpath,array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));
	 ...
	 * @param $options
	 */
	function market($app = '', $options = array(), $img_dest='') {
         $imgpath = $this->base_url . urlencode('market://search?q=' . $app) . $this->_optionsString($options); 
          if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }   
		return $this->Html->image($imgpath,array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));  
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function event($event = array(), $options = array(), $img_dest='') {
		$ret = 'BEGIN:VEVENT';
    ...
    /**
    * function to Download qr code images from  remote server to save at local server 
    * 
    * @param mixed $inPath     - Remote server image path
    * @param mixed $outPath     - Local server image path 
    */
	 ...
	 * @param array $options options array, see helper vars for description of parameters
	 */
	function wifi($wifi = array(), $options = array(), $img_dest='') {
		if (!isset($wifi['ssid'])) {
			$wifi['ssid'] = '';
		}
		...
		$imgpath =$this->base_url . urlencode('wifi:' . $wifi['ssid'] . ',' . $wifi['type'] . ',' . $wifi['password']) . $this->_optionsString($options);
         if($img_dest!='') {
           $this->save_image($imgpath, $img_dest);
        }
        return $this->Html->image($imgpath,array('id'=>'qrcodeimg', 'class'=>"qrcodeimgcls", "alt"=>"Qr Code"));  
